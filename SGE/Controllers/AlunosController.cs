...
namespace SGE.Controllers
{
    public class AlunosController : Controller
    {
        ...
        public async Task<IActionResult> AlterarFoto(Guid id, IFormFile novaFoto)
        {
            ...
            {
                var fileName = aluno.AlunoId.ToString(); //Gera um novo nome pra imagem
                var fileExtension = Path.GetExtension(novaFoto.FileName);  //pega a extensão do arquivo
                var newFileName = fileName + fileExtension; //Novo nome doa rquivo
                var filePath = Path.Combine(Directory.GetCurrentDirectory(), "Data\\Content\\Photo", newFileName); //Caminho do arquivo
            ...
            Guid idTipo = _context.TiposUsuario.Where(a => a.Tipo == "Aluno").FirstOrDefault().TipoUsuarioId; // Busca o id do tipo de usuario

            ViewData["TipoUsuarioId"] = idTipo;  //Exibe o id do tipo de usuario da view
